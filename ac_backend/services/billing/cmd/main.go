...
import (
	...
	"alphacare/ac_backend/services/billing/server"
	"fmt"
	"net"
...
func main() {
	...
	s := grpc.NewServer()
	pb.RegisterBillingServer(s, server.NewServer(
		getDBConn(),
		viper.GetViper(),
	))
	log.Infof("billing svc listening to port: %v", viper.GetString("app.server.port"))
