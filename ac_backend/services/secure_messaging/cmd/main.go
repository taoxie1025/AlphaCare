...
import (
	...
	"alphacare/ac_backend/services/secure_messaging/model"
	"alphacare/ac_backend/services/secure_messaging/server"
	"fmt"
	"net"
...
func getDBConn() *gorm.DB {
	...
	db.Table(server.SecureMessagingTable).AutoMigrate(&model.Message{})
	return db
}
...
func main() {
	...
	s := grpc.NewServer()
	pb.RegisterSecureMessagingServiceServer(s, server.NewServer(
		getDBConn(),
	))
	log.Infof("Secure Messaging svc listening to port: %v", viper.GetString("app.server.port"))
