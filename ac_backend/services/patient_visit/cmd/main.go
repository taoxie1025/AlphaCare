...
import (
	...
	"alphacare/ac_backend/services/patient_visit/server"
	"fmt"
	"net"
...
func getDBConn() *gorm.DB {
	...
	db.Table(server.PatientVisitTable).AutoMigrate(&model.PatientVisit{})
	return db
}
...
func main() {
	...
	s := grpc.NewServer()
	pb.RegisterPatientVisitServiceServer(s, server.NewServer(
		getDBConn(),
	))
	log.Infof("patient visit svc listening to port: %v", viper.GetString("app.server.port"))
